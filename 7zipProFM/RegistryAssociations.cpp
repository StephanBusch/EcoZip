...
static const TCHAR *k7zePrefix = TEXT("7-ZipPro.");

static CSysString GetExtProgramKeyName(const CSysString &ext)
{
  return CSysString(k7zePrefix) + ext;
}
...
bool CShellExtInfo::IsIt7ZipPro() const
{
  return IsString1PrefixedByString2_NoCase(GetUnicodeString(ProgramKey), GetUnicodeString(k7zePrefix));
}
...
LONG DeleteShellExtensionInfo(HKEY hkey, const CSysString &ext)
{
  ...
  // then we delete only 7-ZipPro.* key.
