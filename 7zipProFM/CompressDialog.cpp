    ...
    if (!Info.VolumeSizes.IsEmpty())
    {
      ...
      if (volumeSize < (100 << 10))
      {
        ...
        if (::MessageBoxW(*this, MyFormatNew(IDS_SPLIT_CONFIRM, s),
            L"7-ZipPro", MB_YESNOCANCEL | MB_ICONQUESTION) != IDYES)
          return;
      }
    }
  }
...
static LPCWSTR kHelpTopic = L"fme/plugins/7-ZipPro/add.htm";
    ...
    case kPPMd:
    {
      ...
      for (unsigned i = 20; i < 31; i++)
        for (unsigned j = 0; j < 2; j++)
        {
          ...
          UInt64 decomprSize;
          UInt64 requiredComprSize = GetMemoryUsage(dict, decomprSize);
          if (dict <= defaultDict && requiredComprSize <= maxRamSize || m_cmbDictionary.GetCount() == 0)
            m_cmbDictionary.SetCurSel(m_cmbDictionary.GetCount() - 1);
        }
...
void CCompressDialog::SetSolidBlockSize()
{
  ...
  index = (int)m_cmbSolid.AddString(LangString(IDS_COMPRESS_NON_SOLID));
  ...
  index = (int)m_cmbSolid.AddString(LangString(IDS_COMPRESS_SOLID));
