...
HRESULT CContentsView::OpenItemAsArchive(IInStream *inStream,
...
HRESULT CContentsView::OpenItemAsArchive(const UString &relPath, const UString &arcFormat, bool &encrypted)
{
  CTempFileInfo tfi;
  tfi.RelPath = relPath;
...
HRESULT CContentsView::OpenItemAsArchive(int index, const wchar_t *type)
{
...
void CContentsView::OpenItemInArchive(int index, bool tryInternal, bool tryExternal, bool editMode, bool useEditor, const wchar_t *type)
{
  ...
  unsigned threadID;
  HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, &MyThreadFunction, tpi, 0, &threadID);
  if (hThread == NULL)
    throw 271824;
  tempDirectory.DisableDeleting();
