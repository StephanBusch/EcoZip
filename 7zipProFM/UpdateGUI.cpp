...
#include "CPP/7zip/UI/FileManager/StringUtils.h"
...
static bool IsThereMethodOverride(bool is7ze, const UString &propertiesString)
{
    ...
    bool /* sfxMode */)
{
  ...
  {
    if (!method.IsEmpty())
      AddProp(properties, is7ze ? L"0": L"m", method);
    ...
    CUpdateCallbackGUI *callback, HWND hwndParent)
{
  ...
  */
  ...
  bool is7ze = archiverInfo.Name.IsEqualTo_Ascii_NoCase("7ze");
  bool methodOverride = IsThereMethodOverride(is7ze, di.Options);
  ...
  if (is7ze) {
    CProperty property;
    property.Name = L"LoadConfig";
    property.Value = di.LoadConfig ? L"1" : L"0";
