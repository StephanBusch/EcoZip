...
static const char *g_Exts =
  " 7z 7ze xz lzma ace arc arj bz tbz bz2 tbz2 cab deb gz tgz ha lha lzh lzo lzx pak rar rpm sit zoo"
  " zip jar ear war msi"
...
struct CAnalysis
{
  ...
  const char *GetGroup(UInt32 index, const CUpdateItem &ui);
};
...
const char *CAnalysis::GetGroup(UInt32 index, const CUpdateItem &ui)
{
  int slashPos = ui.Name.ReverseFind_PathSepar();
  int dotPos = ui.Name.ReverseFind_Dot();
    ...
    if (needReadFile && Callback)
    {
      ...
      }
      {
        ...
        if (result == S_OK && stream)
        {
          ...
          if (result == S_OK)
          {
            CFilterMode filterModeTemp;
            const char *pszGroup = NULL;
            if (Parse_EXE(Buffer, size, &filterModeTemp))       pszGroup = "exe";
            else if (Parse_ELF(Buffer, size, &filterModeTemp))  pszGroup = "elf";
            else if (Parse_MACH(Buffer, size, &filterModeTemp)) pszGroup = "mach";
            else if (Parse_WAV(Buffer, size, &filterModeTemp))  pszGroup = "wav";
            else return NULL;
            if (filterModeTemp.Delta == 0)
            {
              filterModeTemp.SetDelta();
              if (filterModeTemp.Delta != 0 && filterModeTemp.Id != k_Delta)
              {
...
{
  bool bLoadConfig = this->bLoadConfig;
  unsigned kNumGroupsMax = _groups.groups.Size() * 2;
  if (kNumGroupsMax == 0 || !bLoadConfig) {
    kNumGroupsMax = 4;
    bLoadConfig = false;
  }
  unsigned numFiles = updateItems.Size();
  ...
  */
  ...
  if (bLoadConfig) {
    while (groups.Size() < kNumGroupsMax)
      groups.AddNew();
  }
  ...
  if (db != 0)
  {
    ...
    for (i = 0; i < db->NumFolders; i++)
    {
      ...
      // Need check later
      unsigned groupIndex = bLoadConfig ? _groups.getGroupIndex(f.Coders[0].MethodID) * 2 + (isEncrypted ? 1 : 0)
	      : Get_FilterGroup_for_Folder(filters, f);
  ...
  if (bLoadConfig) {
    for (unsigned i = 0; i < kNumGroupsMax; i++) {
      CFilterMode2 fm;
      if (i & 1)
        fm.Encrypted = true;
      filters.Add(fm);
    ...
    bool sortByType = (numFiles > 1);
    for (unsigned i = 0; i < numFiles; i++)
      refItems.Add(CRefItem(i, updateItems[i], sortByType));
    ...
    for (unsigned i = 0; i < numFiles; i++)
    {
      const CUpdateItem &ui = updateItems[refItems[i].Index];
      if (!ui.NewData || !ui.HasStream())
        continue;

      const char * pszGroup = analysis.GetGroup(i, ui);
      int nGroup = -1;
      if (pszGroup != NULL) {
        nGroup = _groups.getDefaultGroupIndex(pszGroup);
        if (nGroup >= 0)
          nGroup = nGroup * 2 + (method.PasswordIsDefined ? 1 : 0);
      }
      if (nGroup < 0)
        nGroup = _groups.getGroupIndex(ui.Name) * 2 + (method.PasswordIsDefined ? 1 : 0);

      groups[nGroup].Indices.Add(refItems[i].Index);
    }
  }
  else {
    // ---------- Split files to groups ----------
  ...
  UString strGlobalPassword = (getPasswordSpec == NULL) ?
    options.Method->Password : getPasswordSpec->Password;
//   bool bGlobalPasswordIsDefined = options.Method->PasswordIsDefined;

  #else

  UString strGlobalPassword = options.Method->Password;
//   bool bGlobalPasswordIsDefined = options.Method->PasswordIsDefined;
  ...
  */
  ...
  for (unsigned groupIndex = 0; groupIndex < filters.Size(); groupIndex++)
  {
    ...
    if (bLoadConfig) {
      unsigned groupIndexRef = filterMode.GroupIndex;
      unsigned nGroup = groupIndexRef / 2;
      if (!_groups.groups[nGroup].isValid())
        nGroup = _groups.defaultGroup;
      if (!(groupIndexRef & 1) && _groups.groups.Size() > nGroup) {
        method.Init();
        RINOK(SetProperties(&_groups.groups[nGroup]));

        HRESULT res = SetMainMethod(method
          #ifndef _7ZIP_ST
          , _numThreads
          #endif
          );
        ...
        method.MultiThreadMixer = _useMultiThreadMixer;
        #endif
      }
      ...
      */
      ...
      if (groupIndexRef & 1)//IsEncryptedGroup(groupIndex))
      {
        if (!method.PasswordIsDefined)
        {
          method.Password = strGlobalPassword;
