...
HRESULT CHandler::SetMethodName(CCompressionMethodMode &methodMode,
    ...
    )
{
  ...
  FOR_VECTOR (i, methods)
  {
    ...
    oneMethodInfo.MethodName = methodName;
...
STDMETHODIMP CHandler::SetProperties(const wchar_t * const *names, const PROPVARIANT *values, UInt32 numProps)
{
  ...
  for (UInt32 i = 0; i < numProps; i++)
  {
    ...
    if (name.Compare(L"loadconfig") == 0) {
      bLoadConfig = (value.boolVal != VARIANT_FALSE);
    }
    else {
      ...
      if (name[0] != 'f') {
        ...
        //UString realName = name.Ptr(index);
        if (index == 0) {
          _defaultPropNames.Add(name);
          _defaultPropValues.Add(value);
        }
      }
    }
  }

  RINOK(AfterSetProperties());

  return S_OK;
  COM_TRY_END
}

STDMETHODIMP CHandler::SetProperties(Group *group)
{
  COM_TRY_BEGIN
  _bonds.Clear();
  InitProps();

  AStringVector propNames;
  CObjectVector<NWindows::NCOM::CPropVariant> propValues;

  FOR_VECTOR(i, group->codecs) {
    propNames.Add(group->codecs[i].argName);
    propValues.Add(NWindows::NCOM::CPropVariant(GetUnicodeString(group->codecs[i].argValue)));
  }
  FOR_VECTOR (i, group->globals) {
    AString name = group->globals[i];
    name.MakeLower_Ascii();
    if (name[0] == 'm')
      name.Delete(0);
    propNames.Add(name);
    propValues.Add(NWindows::NCOM::CPropVariant());
  }

  FOR_VECTOR (i, propNames)
  {
    UString &name = GetUnicodeString(propNames[i]);
    name.MakeLower_Ascii();
    if (name.IsEmpty())
      return E_INVALIDARG;

    const PROPVARIANT &value = propValues[i];
...
STDMETHODIMP CHandler::AfterSetProperties()
{
