...
#include "7zeProperties.h"
#include "../../Compress/LibbscCoder.h"
#include "../../Compress/csc32/csc32CCsc.h"
  ...
  for (; numCoders != 0; numCoders--, needSpace = true)
  {
    ...
    if (id64 <= (UInt32)0xFFFFFFFF)
    {
      ...
      else if (id == k_LIBBSC)
      {
        name = "LIBBSC";
        if (propsSize >= 5) {
          CLibbscProps curprops;
          curprops.features = props[0];
          curprops.paramBlockSize = props[1] * (1024 * 1024);
          curprops.features |= (((props[2] >> 5) & 0x7) == LIBBSC_CODER_QLFC_STATIC) ? 0 : PARAM_CODER_QLFC_ADAPTIVE;
          curprops.paramBlockSorter = (props[2] & 0x1f);
          curprops.paramLZPMinLen = props[3];
          curprops.paramLZPHashSize = props[4];

          char *dest = s;
          *dest++ = 'b';
          ConvertUInt32ToString((curprops.paramBlockSize + MB - 1) / MB, dest);
          dest += MyStringLen(dest);
          *dest++ = ':';
          if (curprops.paramCoder() != LIBBSC_CODER_QLFC_STATIC) {
            dest = MyStpCpy(dest, "e=");
            ConvertUInt32ToString(curprops.paramCoder(), dest);
            dest += MyStringLen(dest);
          }
          if (curprops.paramBlockSorter != LIBBSC_BLOCKSORTER_BWT) {
            dest = MyStpCpy(dest, ":m=");
            ConvertUInt32ToString(curprops.paramBlockSorter, dest);
            dest += MyStringLen(dest);
          }
          if (curprops.paramSortingContexts != LIBBSC_CONTEXTS_FOLLOWING) {
            dest = MyStpCpy(dest, ":c=");
            if (curprops.paramSortingContexts == LIBBSC_CONTEXTS_PRECEDING)
              *dest++ = 'p';
            else if (curprops.paramSortingContexts == LIBBSC_CONTEXTS_AUTODETECT)
              *dest++ = 'a';
          }
          if (!curprops.paramEnableFastMode())
            dest = MyStpCpy(dest, ":f");
          if (!curprops.paramEnableMultiThreading())
            dest = MyStpCpy(dest, ":t");
          if (curprops.paramEnableLargePages())
            dest = MyStpCpy(dest, ":p");
          if (curprops.paramEnableCUDA())
            dest = MyStpCpy(dest, ":g");
          if (curprops.paramEnableSegmentation())
            dest = MyStpCpy(dest, ":s");
          if (curprops.paramEnableReordering())
            dest = MyStpCpy(dest, ":r");
          if (!curprops.paramEnableLZP())
            dest = MyStpCpy(dest, ":l");
          if (curprops.paramLZPHashSize != LIBBSC_DEFAULT_LZPHASHSIZE) {
            dest = MyStpCpy(dest, ":h");
            ConvertUInt32ToString(curprops.paramLZPHashSize, dest);
            dest += MyStringLen(dest);
          }
          if (curprops.paramLZPMinLen != LIBBSC_DEFAULT_LZPMINLEN) {
            dest = MyStpCpy(dest, ":z=");
            ConvertUInt32ToString(curprops.paramLZPMinLen, dest);
            dest += MyStringLen(dest);
          }
          if (*dest != '\0')
            *dest = '\0';
      ...
      else if (id == k_CSC32 && propsSize >= CSC32_SETTING_LENGTH) {
        name = "CSC32";
        CSCSettings *curprops = (CSCSettings *)(const Byte *)props;
        char *dest = s;
        dest = MyStpCpy(dest, "m=");
        ConvertUInt32ToString(curprops->lzMode, dest);
        dest += MyStringLen(dest);
        if (curprops->wndSize < MB) {
          dest = MyStpCpy(dest, ":dk=");
          ConvertUInt32ToString((curprops->wndSize + KB - 1) / KB, dest);
          dest += MyStringLen(dest);
        } else {
          dest = MyStpCpy(dest, ":d=");
          ConvertUInt32ToString((curprops->wndSize + MB - 1) / MB, dest);
          dest += MyStringLen(dest);
        }
        if (!curprops->EXEFilter()) {
          dest = MyStpCpy(dest, ":fo");
        }
      }
    }
