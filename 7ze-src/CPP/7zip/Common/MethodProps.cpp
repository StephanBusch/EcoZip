...
static const CNameToPropID g_NameToPropID[] =
{
  ...
  { VT_UI4, "reduceSize" },
  // LIBBSC
  { VT_UI4, "LibbscB" },         // kLibbscBlockSize
  { VT_UI4, "LibbscM" },         // kLibbscBlockSorter
  { VT_UI4, "LibbscE" },         // kLibbscCoder
  { VT_BSTR, "LibbscC" },        // kLibbscSortingContexts
  { VT_BOOL, "LibbscT" },        // kLibbscEnableParallelProcessing
//   { VT_BOOL, "LibbscT" },        // kLibbscEnableMultiThreading
  { VT_BOOL, "LibbscF" },        // kLibbscEnableFastMode
  { VT_BOOL, "LibbscP" },        // kLibbscEnableLargePages
  { VT_BOOL, "LibbscG" },        // kLibbscEnableCUDA
  { VT_BOOL, "LibbscS" },        // kLibbscEnableSegmentation
  { VT_BOOL, "LibbscR" },        // kLibbscEnableReordering
  { VT_BOOL, "LibbscL" },        // kLibbscEnableLZP
  { VT_UI4, "LibbscH" },         // kLibbscLZPHashSize
  { VT_UI4, "LibbscZ" },         // kLibbscLZPMinLen
  { VT_UI4, "CscM" },            // kCscM
  { VT_UI4, "CscD" },            // kCscD
  { VT_UI4, "CscDK" },           // kCscDK
  { VT_BOOL, "CscFO" },          // kCscFO
};
...
static bool ConvertProperty(const PROPVARIANT &srcProp, VARTYPE varType, NCOM::CPropVariant &destProp)
{
  ...
  {
    if (srcProp.vt == VT_BSTR)
      destProp = srcProp.bstrVal;
    else
      destProp = srcProp;
    return true;
  }
  if (varType == VT_UI4)
  {
    if (srcProp.vt == VT_BSTR) {
      UInt32 res;
      ParseStringToUInt32(srcProp.bstrVal, res);
      destProp = res;
      return true;
    }
  }
  if (varType == VT_BOOL)
  {
    ...
    if (srcProp.vt == VT_BSTR) {
      UInt32 v;
      ParseStringToUInt32(srcProp.bstrVal, v);
      res = (v != 0);
    } else if (PROPVARIANT_to_bool(srcProp, res) != S_OK)
      return false;
    destProp = res;
    return true;
  }
