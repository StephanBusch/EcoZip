...
{	
	// iomode (i/o mode)
	// 0 -> reading with iostream
	// 1 -> writing with iostream
	// 2 -> reading with ISequentialInStream
	// 3 -> writing with abytewriter
...
aricoder::aricoder( ISequentialInStream *inStream )
{	
	int i;
  ...
  writer = NULL;
	...
	if ( mode == 2 ) { // mode is reading / decoding
		// code buffer has to be filled before starting decoding
		for ( i = 0; i < CODER_USE_BITS; i++ )
			ccode = ( ccode << 1 ) | read_bit();
	} // mode is writing / encoding otherwise
...
aricoder::aricoder( abytewriter *writer )
{	
	// set initial values
	ccode	= 0;
	clow	= 0;
...
{
	if ( mode == 1 || mode == 3 ) { // mode is writing / encoding
...
extern unsigned char* ex_data;	// extra data
extern int            ex_data_size;	// size of extra data
extern int            ex_valid_size;	// size of valid extra data
extern int            ex_next_offset; // offset of found mp3 file or next section
extern int            ex_read_limit; // size of reading limit
...
//   UInt32 processedSize;
	...
	if ( cbit == 0 ) {
// 		if ( FAILED(((ISequentialInStream *)sptr)->Read( &bbyte, 1, &processedSize )) ||
//       processedSize == 0 ) // read next byte if available
...
model_s::model_s( int max_s, int max_c, int max_o, int c_lim )
{
	...
	null_table->max_symbol = (unsigned short)max_symbol;
...
void model_s::update_model( int symbol )
{
	...
	if ( symbol >= 0 ) {
		for ( local_order = ( current_order < 0 ) ? 0 : current_order;
			...
			if ( symbol >= context->max_symbol ) context->max_symbol = (unsigned short)(symbol+1);
...
inline void model_s::rescale_table( table_s* context, int scale_factor )
{
	...
	for ( i = lst_symbol - 1; i >= 0; i-- )
		if ( counts[ i ] > 0 ) break;
	context->max_symbol = (unsigned short)(i + 1);
}
