...
INTERN inline bool pmp_write_header(abytewriter *str);
INTERN inline long long read_int(ISequentialInStream *inStream, UInt64 *inSizeProcessed);
INTERN inline HRESULT write_int(ISequentialOutStream *outStream, long long data_size, UInt64 *outSizeProcessed);
/*INTERN */bool prepare_ex_data(ISequentialInStream *inStream, int needed);
INTERN inline bool pmp_read_header(ISequentialInStream *inStream, UInt64 *inSizeProcessed);
...
INTERN inline bool pmp_store_unmute_data( abytewriter *str );
INTERN inline bool pmp_unstore_unmute_data(ISequentialInStream *inStream);
...
// 		kbps  = (int)(( total > 0 ) ? ( acc_mp3size / total ) : acc_mp3size);
...
// 	cr    = ( mp3filesize > 0 ) ? ( 100.0f * pmpfilesize / mp3filesize ) : 0.0f;
// 
// 
// 	if ( verbosity >= 0 ) { // standard UI
// 		if ( verbosity > 1 )
...
INTERN bool reset_buffers( void )
{
  ...
  ex_read_limit = 0;
  if (ex_backup != NULL) free(ex_backup);
  ex_backup = NULL;
  ex_back_size = 0;
...
HRESULT read_mp3(ISequentialInStream *inStream, UInt64 *inSizeProcessed)
{
  ...
  back_writer = new abytewriter(0);
    ...
    /*****************************************************************
     ...
     |<-   pos   ->|                                                 |
     *****************************************************************/
    if (ex_valid_size - pos < FRAME_MIN_SZ) {
      if (ex_data_size - ex_valid_size < FRAME_MIN_SZ) {
        memmove(mp3data, mp3data + pos, ex_valid_size - pos);
        ex_valid_size -= pos;
        pos = 0;
      }
      readSize = ex_data_size - ex_valid_size;
      RINOK(ReadStream(inStream, mp3data + ex_valid_size, &readSize));
      ex_valid_size += (int)readSize;
      if (ex_valid_size - pos < FRAME_MIN_SZ)
        break;
    }

    // read frame, check result
    frame = mp3_read_frame(mp3data + pos, FRAME_MIN_SZ);
		if ( frame == NULL ) {
      break;
// 			if ( errorlevel == 2 ) {
...
// 			}
// 			else break;
		} else if ( frame->frame_size > ex_valid_size - pos ) {
      if (ex_data_size - pos < frame->frame_size) {
        memmove(mp3data, mp3data + pos, ex_valid_size - pos);
        ex_valid_size -= pos;
        pos = 0;
      }
      ...
      RINOK(ReadStream(inStream, mp3data + ex_valid_size, &readSize));
      ex_valid_size += (int)readSize;
      if (ex_valid_size - pos < frame->frame_size) {
    ...
    if ( ( frame->mpeg != mpeg ) || ( frame->layer != layer ) ||
    ...
    back_writer->write_n( mp3data + pos, frame->frame_size );
    *inSizeProcessed += frame->frame_size;
  ...
  ex_next_offset = pos;
    ...
    ex_back_size = back_writer->getpos();
    if (ex_back_size > 0) ex_backup = back_writer->getptr();
    delete(back_writer);
...
INTERN inline int ex_processed_data_size()
{
  if (ex_next_offset < 0) {
    if (ex_valid_size == ex_data_size)
      return ex_data_size - GARBAGE_TOLERANCE / 4;
    else
      return ex_valid_size;
  }
  else
    return ex_next_offset;
}

INTERN inline void ex_move()
{
  int processedSize = ex_processed_data_size();
  memmove(ex_data, ex_data + processedSize, ex_valid_size - processedSize);
  ex_valid_size -= processedSize;
  ex_next_offset -= processedSize;
}
...
HRESULT seek_mp3(ISequentialInStream *inStream, int *processedSize)
{
  ...
  int offset = ex_valid_size;
  readSize = ex_data_size - offset;
  RINOK(ReadStream(inStream, ex_data + offset, &readSize));
  if (offset + readSize == 0) {
    ex_next_offset = -1;
    *processedSize = 0;
    return S_FALSE;
  }

  ex_valid_size = offset + (int)readSize;

  // find first proper mpeg audio frame
  if (ex_next_offset < 0)
    pos = mp3_seek_firstframe(ex_data, ex_valid_size);
  else
    pos = mp3_seek_firstframe(ex_data + 1, ex_valid_size - 1);

  if (pos == -1) {
    ex_next_offset = -1;

    // If data is smaller than ex_data_size, process at once.
    // Or process only half
    *processedSize = ex_processed_data_size();
...
bool write_mp3(ISequentialOutStream *outStream, UInt64 *outSizeProcessed)
{
  ...
  WriteStream(outStream, data_before, data_before_size);
  *outSizeProcessed += data_before_size;
	...
	for ( frame = firstframe; frame != NULL; frame = frame->next ) {
		...
		WriteStream(outStream, fixed, frame->fixed_size);
    WriteStream(outStream, data, frame->frame_size - frame->fixed_size);
    *outSizeProcessed += frame->frame_size;
    data += frame->frame_size - frame->fixed_size;
	}

	// write data after
	if (FAILED(WriteStream(outStream, data_after, data_after_size))) {
    // errormessage if write error
		sprintf( errormessage, "write error, possibly drive is full" );
		errorlevel = 2;
		return false;
	}
  *outSizeProcessed += data_after_size;
...
// 	data_writer->write_n((unsigned char *)pmp_magic, 2);
// 	data_writer->write_n((unsigned char *)&appversion, 1);

	// PMP header data
	if ( !pmp_write_header( data_writer ) ) return false;
	...
	if ( n_bad_first > 0 ) {
		...
		// store fix data
    if (!pmp_store_unmute_data(data_writer)) return false;
	}
	...
	*/
	...
	// PMP magic number & version byte
	if (FAILED(WriteStream(outStream, (unsigned char *)pmp_magic, 2)) ||
...
int check_archive_type(ISequentialInStream *inStream, bool *remained)
{
  ...
  *remained = true;
  ...
  // PMP magic number
  if (memcmp(ex_data + ex_next_offset, pmp_magic, 2) != 0) {
    return 1;
  }
...
bool uncompress_pmp(ISequentialInStream *inStream, UInt64 *inSizeProcessed)
{
	...
	ex_next_offset += 2;
	*inSizeProcessed += 2;
	// Here ex_next_offset should be the same with ex_valid_size

	// version number
	if (!prepare_ex_data(inStream, 1))
		return false;
	hcode = ex_data[ex_next_offset++];
	*inSizeProcessed += 1;

	// compare version number
	if ( hcode != appversion ) {
		sprintf( errormessage, "incompatible file, version v%i.%i",
			hcode / 10, hcode % 10 );
		errorlevel = 2;
		return false;
	}
  ...
  if (!pmp_read_header(inStream, inSizeProcessed)) return false;
	...
	// unstore fix data
	if ( n_bad_first > 0 ) {
		if ( !pmp_unstore_unmute_data( inStream ) ) return false;
        ...
        granule->region_bound[1] = (short)
				...
				if ( granule->block_type != SHORT_BLOCK ) {
					...
					granule->region_bound[0] =
						(short) mp3_bandwidth_bounds[(int)frame->samples][8];
...
INTERN inline bool mp3_mute_frame( mp3Frame* frame )
{
	...
	if ( frame->prev != NULL ) {
		...
		frame->bit_reservoir += (short)frame->prev->aux_size;
			...
			if ( granule->block_type != SHORT_BLOCK ) {
				...
				for ( g = 0, p = 0; g < 4; g++ ) {
					if ( (share>>(3-g)) & 0x1 ) { // shared
						...
						*(scfs++) = (unsigned char)dec->read_bits( sl );
					}
				}
			...
			for ( p = 0, r = 0; r < 3; r++ ) {
				...
				if ( linbits == 0 ) { // without linbits
					...
					for ( ; p < region_bounds[ r ]; p += 2, coefs += 2 ) {
						dec->decode_pair( conv_set, vals );
						for ( i = 0; i < 2; i++ ) if ( vals[i] > 0 ) {
							coefs[i] = (short)(( vals[i] == 15 ) ? 15 + dec->read_bits( linbits ) : vals[i]);
...
INTERN inline bool pmp_write_header(abytewriter *str)
{
	...
	str->write_n( (unsigned char *) header, 4 );
	str->write_n( (unsigned char *) nframes, 4 );
...
INTERN inline bool pmp_read_header(ISequentialInStream *inStream, UInt64 *inSizeProcessed)
{
  ...
  if (!prepare_ex_data(inStream, 8)) {
    sprintf(errormessage, "unexpected end of data");
    errorlevel = 2;
    return false;
  }
  *inSizeProcessed += 8;
  header = ex_data + ex_next_offset;
  nframes = ex_data + ex_next_offset + 4;
  ex_next_offset += 8;
  ex_read_limit -= 8;
...
INTERN inline bool pmp_decode_stereo_ms( aricoder* dec )
{
	...
	for ( mp3Frame* frame = firstframe; frame != NULL; frame = frame->next ) {
		model->shift_context( ctx );
		frame->stereo_ms = (char)decode_ari( dec, model );
		ctx = ( ( ctx << 1 ) | frame->stereo_ms ) & 0xF;
	}
	...
	for ( ch = 0; ch < g_nchannels; ch++ ) {
		...
		for ( granule = firstframe->granules[ch][0]; granule != NULL; granule = granule->next ) {
			...
			if ( !granule->window_switching ) {
				// region 2
				mod_bv->shift_context( 0 );
				granule->big_val_pairs = (short)decode_ari( dec, mod_bv );
				s_r2 = bw_conv[ granule->big_val_pairs << 1 ];
				// region 0
				shift_model( mod_s0, s_r0, s_r2 );
				s_r0 = decode_ari( dec, mod_s0 );
				granule->region0_size = (char)s_r0;
				// region 1
				shift_model( mod_s1, s_r0, s_r2 );
				granule->region1_size = (char)decode_ari( dec, mod_s1 );
        ...
        granule->region_bound[0] = (short)
			...
			granule->region_table[0] = (char)t_r0;
			// region 1 table
			shift_model( mod_t1, t_r0, s_r0 );
			t_r1 = decode_ari( dec, mod_t1 );
			granule->region_table[1] = (char)t_r1;
			// region 2 table
			if ( !granule->window_switching ) {
				shift_model( mod_t2, t_r0, t_r1 );
				t_r2 = decode_ari( dec, mod_t2 );
				granule->region_table[2] = (char)t_r2;
			} else granule->region_table[2] = 0;
			// small values table
			mod_ts->shift_context( ctx_sv );
			t_sv = decode_ari( dec, mod_ts );
      granule->select_htabB = (char)t_sv;
			ctx_sv = ( ( ctx_sv << 1 ) | t_sv ) & 0xF;
			// advance context
			gg_ctx++;
		}
...
INTERN inline bool pmp_decode_coarse_sf( aricoder* dec )
{
	...
	for ( ch = 0; ch < g_nchannels; ch++ ) {
		...
		for ( granule = firstframe->granules[ch][0]; granule != NULL; granule = granule->next ) {
			model->shift_context( ctx );
			c = decode_ari( dec, model );
      granule->coarse_scalefactors = (char)c;
			ctx = ( ( ctx << 1 ) | c ) & 0xF;
		}
	}
...
INTERN inline bool pmp_decode_subblock_gain( aricoder* dec )
{
	...
	for ( ch = 0; ch < g_nchannels; ch++ ) {
		...
		for ( granule = firstframe->granules[ch][0]; granule != NULL; granule = granule->next ) {
			if ( granule->window_switching ) {
				for ( sb = 0; sb < 3; sb++ ) {
          granule->sb_gain[sb] = (char)decode_ari(dec, model);
					model->shift_context( granule->sb_gain[sb] );
				}
	...
	for ( frame = firstframe; frame != NULL; frame = frame->next, bitp = 0 ) {
		for ( gr = 0; gr < 2; gr++ ) {
			for ( ch = 0; ch < g_nchannels; ch++ ) {
					...
					for ( g = 0, p = 0; g < 4; g++ ) {
						...
						if ( ( gr ) & ( shared[ch][g] ) ) { // shared
							...
							for ( ; p < scf_bounds[ g ]; p++ ) { // non shared
								scf[p] = (unsigned char)huffman->read_bits( sl );
					...
					for ( ; p < region_bounds[ r ]; bitc = huffman->get_count() ) {
						...
						for ( i = 0; i < 2; i++, p++ ) if ( abs[p] > 0 ) {
							if ( linbits > 0 ) if ( abs[p] == 15 ) {
								...
								lbt[p] = (unsigned short)huffman->read_bits( linbits );
				...
				granule->sv_bound = (short)p; // set sv_bound
				if ( !sbl )
				...
				if ( p >= granule->region_bound[ 2 ] ) { // for proper files: encode actual sv bound
					...
					// fix bounds
					granule->region_bound[ 2 ] = (short)p;
					for ( i = 1; i >= 0; i-- ) if ( p < granule->region_bound[i] )
						granule->region_bound[i] = (short)p;
					else break;
				}
		...
		for ( pna_c = pad_and_aux, i = n; i >= 8; i -= 8 )
			*(pna_c++) = (unsigned char)huffman->read_bits( 8 );
		*pna_c = (unsigned char)huffman->read_bits( i );
	...
	for ( frame = firstframe; frame != NULL; frame = frame->next, bitp = 0 ) {
		...
		for ( gr = 0; gr < 2; gr++ ) {
			for ( ch = 0; ch < g_nchannels; ch++ ) {
				...
				if ( !sbl ) {
					...
					for ( g = 0, p = 0; g < 4; g++ ) {
						...
						if ( ( gr ) & ( shared[ch][g] ) ) { // shared
							...
							for ( ; p < scf_bounds[ g ]; p++ ) { // non shared
								shift_model( mod_sfc, ctx_scf, scf_prev[p] );
								scf[p] = (unsigned char)decode_ari( dec, mod_sfc );
				...
				for ( p = granule->sv_bound-1; p >= rlb; p-- ) {
					shift_model( mod_asc, ctx_pat, ctx_h_abs[p] );
					abs[p] = (unsigned char)decode_ari( dec, mod_asc ); // absolutes
					ctx_pat = ( (ctx_pat<<1) | abs[p] ) & 0xF;
					ctx_abs = ( 2 * abs[p] + ctx_abs + 2 ) / 3;
					if ( abs[p] == 1 ) {
						shift_model( mod_sgc, ctx_h_abs[p], ctx_h_sgn[p] );
						sgn[p] = (unsigned char)decode_ari( dec, mod_sgc ); // signs
					}
				}
				...
				for ( r = 2; r >= 0; r-- ) {
					...
					for ( ; p >= rlb; p-- ) {
						shift_model( mod_abc, ctx_abs, ctx_h_abs[p] );
						abs[p] = (unsigned char)decode_ari( dec, mod_abc ); // absolutes
						ctx_abs = ( 2 * abs[p] + ctx_abs + 2 ) / 3;
						if ( abs[p] > 0 ) {
							shift_model( mod_sgc, ctx_h_abs[p], ctx_h_sgn[p] );
							sgn[p] = (unsigned char)decode_ari( dec, mod_sgc ); // signs
							if ( linbits > 0 ) if ( abs[p] == 15 ) {
								// loop invariant condition (-funswitch-loops)
								mod_lnc->shift_context( ctx_h_len[p] );
								len[p] = (unsigned char)decode_ari( dec, mod_lnc ); // bitlengths
								if ( len[p] > 0 ) {
									for ( lbt[p] = 1, i = len[p] - 2; i >= 0; i-- ) {
										shift_model( mod_res, len[p], i ); // bit residuals
										lbt[p] = (unsigned short)(( lbt[p] << 1 ) | decode_ari( dec, mod_res ));
									}
								} else lbt[p] = 0;
							}
						}
          ...
          huffman->write_bit((unsigned char)c);
					ctx_bst = ( (ctx_bst<<1) | c ) & 0xF;
				}


				// ---> GRANULE FINISHED! <---
				granule->main_data_bit = (short)huffman->get_count();
		...
		if ( i_bitrate == -1 ) {
			...
			frame->bits = (char)decode_ari( dec, mod_btr );
		}
		// calculate size of frame - use the lookup table
...
INTERN inline bool pmp_store_unmute_data( abytewriter *str )
{
	...
	str->write_n( unmute_data, unmute_data_size );
...
INTERN inline bool pmp_unstore_unmute_data(ISequentialInStream *inStream)
{
  ...
  if (!prepare_ex_data(inStream, 1))
	...
	*unmute_data = n;
	...
	// check for eof
  if (!prepare_ex_data(inStream, unmute_data_size - 1)) {
		sprintf( errormessage, "unexpected end of data" );
		errorlevel = 2;
		return false;
	}

  memcpy(unmute_data + 1, ex_data + ex_next_offset, unmute_data_size - 1);
  ex_next_offset += unmute_data_size - 1;
...
// 
// 		for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 			line[ (frame->n-i) * 2 + 0 ] = (unsigned char)(frame->granules[0][0]->main_data_bit >> 4);
// 			line[ (frame->n-i) * 2 + 1 ] = (unsigned char)(frame->granules[0][1]->main_data_bit >> 4);
// 		}
// 		fwrite( line, 1, img_width, fp ); // 27 (MAIN DATA BIT)
// 		if ( g_nchannels == 2 ) {
// 			for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 				line[ (frame->n-i) * 2 + 0 ] = (unsigned char)(frame->granules[1][0]->main_data_bit >> 4);
// 				line[ (frame->n-i) * 2 + 1 ] = (unsigned char)(frame->granules[1][1]->main_data_bit >> 4);
// 			}
// 		}
// 		fwrite( line, 1, img_width, fp ); // 28 (MAIN DATA BIT)
// 
// 		for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 			line[ (frame->n-i) * 2 + 0 ] = (unsigned char)(frame->granules[0][0]->big_val_pairs >> 1);
// 			line[ (frame->n-i) * 2 + 1 ] = (unsigned char)(frame->granules[0][1]->big_val_pairs >> 1);
// 		}
// 		fwrite( line, 1, img_width, fp ); // 29 (BIG VALUES)
// 		if ( g_nchannels == 2 ) {
// 			for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 				line[ (frame->n-i) * 2 + 0 ] = (unsigned char)(frame->granules[1][0]->big_val_pairs >> 1);
// 				line[ (frame->n-i) * 2 + 1 ] = (unsigned char)(frame->granules[1][1]->big_val_pairs >> 1);
// 			}
// 		}
// 		fwrite( line, 1, img_width, fp ); // 30 (BIG VALUES)
// 
// 		for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 			line[ (frame->n-i) * 2 + 0 ] = (unsigned char)(frame->granules[0][0]->share << 4);
// 			line[ (frame->n-i) * 2 + 1 ] = (unsigned char)(frame->granules[0][1]->share << 4);
// 		}
// 		fwrite( line, 1, img_width, fp ); // 31 (SHARING)
// 		if ( g_nchannels == 2 ) {
// 			for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 				line[ (frame->n-i) * 2 + 0 ] = (unsigned char)(frame->granules[1][0]->share << 4);
// 				line[ (frame->n-i) * 2 + 1 ] = (unsigned char)(frame->granules[1][1]->share << 4);
// 			}
// 		}
// 		fwrite( line, 1, img_width, fp ); // 32 (SHARING)
// 
// 		for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 			line[ (frame->n-i) * 2 + 0 ] = (unsigned char)frame->granules[0][0]->global_gain;
// 			line[ (frame->n-i) * 2 + 1 ] = (unsigned char)frame->granules[0][1]->global_gain;
// 		}
// 		fwrite( line, 1, img_width, fp ); // 33 (GLOBAL GAIN)
// 		if ( g_nchannels == 2 ) {
// 			for ( frame = frame0; frame != frame1; frame = frame->next ) {
// 				line[ (frame->n-i) * 2 + 0 ] = (unsigned char)frame->granules[1][0]->global_gain;
// 				line[ (frame->n-i) * 2 + 1 ] = (unsigned char)frame->granules[1][1]->global_gain;
// 			}
// 		}
...
//                   temp_sc[r] += (unsigned char)preemphasis_table[i];
...
// 					if ( g_nchannels == 1 ) {
// 						data_sc[i][ ( 36 * width_sc ) + n_sc ] = (unsigned char)granule->global_gain;
// 						data_sc[i][ ( 37 * width_sc ) + n_sc ] = (unsigned char)granule->global_gain;
// 					} else if ( ch == 0 ) {
// 						data_sc[i][ ( 36 * width_sc ) + n_sc ] = (unsigned char)granule->global_gain;
// 						data_sc[i][ ( 37 * width_sc ) + n_sc ] = (unsigned char)frame->stereo_ms * 255;
// 					} else {
// 						data_sc[i][ ( 38 * width_sc ) + n_sc ] = (unsigned char)frame->stereo_ms * 255;
// 						data_sc[i][ ( 39 * width_sc ) + n_sc ] = (unsigned char)granule->global_gain;
// 					}
// 				}
// 
// 
...
//               for (r = 0; r < 576; r++)	temp_cf[r] = (unsigned char)(
...
// 								temp_cf[r] = (unsigned char)((15-tmp) * 16);
// 							break;
// 					}
...
// 					if ( g_nchannels == 1 ) {
// 						data_cf[i][ ( 580 * width_cf ) + n_cf ] = (unsigned char)granule->global_gain;
// 						data_cf[i][ ( 581 * width_cf ) + n_cf ] = (unsigned char)granule->global_gain;
// 					} else if ( ch == 0 ) {
// 						data_cf[i][ ( 580 * width_cf ) + n_cf ] = (unsigned char)granule->global_gain;
// 						data_cf[i][ ( 581 * width_cf ) + n_cf ] = frame->stereo_ms * 255;
// 					} else {
// 						data_cf[i][ ( 582 * width_cf ) + n_cf ] = frame->stereo_ms * 255;
// 						data_cf[i][ ( 583 * width_cf ) + n_cf ] = (unsigned char)granule->global_gain;
// 					}
// 				}
// 			} else {
// 				// no scalefactors
...
HRESULT compress_replacement(ISequentialOutStream *outStream, int which, UInt64 *inSizeProcessed, UInt64 *outSizeProcessed)
{
  ...
  UInt32 processedSize = ex_processed_data_size();
  UInt32 nReduceSize = (which == 0) ? processedSize : ex_back_size;
  unsigned char * data = (which == 0) ? ex_data : ex_backup;
