...
#include "./coder/common/rangecoder.h"
#include "./coder/common/tables.h"
#include "./coder/common/predictor.h"
...
unsigned char * bsc_qlfc_transform(const unsigned char * RESTRICT input, unsigned char * RESTRICT buffer, int n, unsigned char * RESTRICT MTFTable)
{
    ...
    for (int i = 0; i < ALPHABET_SIZE; ++i) MTFTable[i] = (unsigned char)i;
    ...
    for (const unsigned char * inputEnd = input + inputSize; input < inputEnd;)
    {
        ...
        if (avgRank < 32)
        {
            ...
            else
            {
                ...
                int bitRankSize = bsc_log2_256(rank); rankHistory[currentChar] = (unsigned char)bitRankSize;
        ...
        {
            rankHistory[currentChar] = (unsigned char)bsc_log2_256(rank);
        ...
        else
        {
            ...
            int bitRunSize = bsc_log2(runSize); runHistory[currentChar] = (unsigned char)((runHistory[currentChar] + 3 * bitRunSize + 3) >> 2);
...
int bsc_qlfc_adaptive_decode(const unsigned char * input, unsigned char * output, QlfcStatisticalModel * model)
{
    ...
    for (int rank = 0; rank < ALPHABET_SIZE; ++rank)
    {
        ...
        MTFTable[rank] =  (unsigned char)currentChar;
    ...
    for (int i = 0; i < n;)
    {
        ...
        if (avgRank < 32)
        {
            if (coder.DecodeBit(mixer->Mixup(*charPredictor, *statePredictor, *staticPredictor)))
            {
                ...
                rankHistory[currentChar] = (unsigned char)bitRankSize;
        ...
        if (coder.DecodeBit(mixer->Mixup(*charPredictor, *statePredictor, *staticPredictor)))
        {
            ...
            runHistory[currentChar] = (unsigned char)((runHistory[currentChar] + 3 * bitRunSize + 3) >> 2);
        ...
        for (; runSize > 0; --runSize) output[i++] = (unsigned char)currentChar;
    }
