...
static const CContextMenuCommand g_Commands[] =
{
  ...
  {
    NContextMenuFlags::kCompressTo7ze,
    CZipContextMenu::kCompressTo7ze,
    L"CompressTo7ze",
    IDS_CONTEXT_COMPRESS_TO,
  },
  {
    NContextMenuFlags::kCompressTo7zeEmail,
    CZipContextMenu::kCompressTo7zeEmail,
    L"CompressTo7zeEmail",
    IDS_CONTEXT_COMPRESS_TO_EMAIL,
  },
};
...
static const char *kNoOpenAsExtensions =
  " 7z 7ze arj bz2 cab chm cpio flv gz lha lzh lzma rar swm tar tbz2 tgz wim xar xz z zip ";
*/
      ...
      UINT commandIDFirst, UINT commandIDLast, UINT flags)
{
  ...
  if (_fileNames.Size() > 0 && currentCommandID + 10 <= commandIDLast)
  {
    ...
    UString arcName7ze = arcName + L".7ze";
    ...
    // CompressTo7ze
    if (contextMenuFlags & NContextMenuFlags::kCompressTo7ze &&
        !arcName7ze.IsEqualTo_NoCase(fs2us(fi0.Name)))
    {
      CCommandMapItem commandMapItem;
      UString s;
      FillCommand(kCompressTo7ze, s, commandMapItem);
      if (_dropMode)
        commandMapItem.Folder = _dropPath;
      else
        commandMapItem.Folder = fs2us(folderPrefix);
      commandMapItem.ArcName = arcName7ze;
      commandMapItem.ArcType.SetFromAscii("7ze");
      MyFormatNew_ReducedName(s, arcName7ze);
      MyInsertMenu(popupMenu, subIndex++, currentCommandID++, s, bitmap);
      _commandMap.Add(commandMapItem);
    }
    ...
    #ifdef EMAIL_SUPPORT
    // CompressTo7zeEmail
    if ((contextMenuFlags & NContextMenuFlags::kCompressTo7zeEmail) != 0  && !_dropMode)
    {
      CCommandMapItem commandMapItem;
      UString s;
      FillCommand(kCompressTo7zeEmail, s, commandMapItem);
      commandMapItem.ArcName = arcName7ze;
      commandMapItem.ArcType.SetFromAscii("7ze");
      MyFormatNew_ReducedName(s, arcName7ze);
      MyInsertMenu(popupMenu, subIndex++, currentCommandID++, s, bitmap);
  ...
  if (ci.Cascaded.Val)
  {
    ...
    mi.StringValue.SetFromAscii("EcoZip"); // LangString(IDS_CONTEXT_POPUP_CAPTION);
    mi.hbmpUnchecked = bitmap;
...
static UString Get7zeFmPath()
{
  return fs2us(NWindows::NDLL::GetModuleDirPrefix()) + L"EcoZipFM.exe";
}
...
STDMETHODIMP CZipContextMenu::InvokeCommand(LPCMINVOKECOMMANDINFO commandInfo)
{
  ...
  try
  {
    switch (cmdID)
    {
      case kOpen:
      {
        ...
        MyCreateProcess(Get7zeFmPath(), params);
        break;
      }
      case kExtract:
      case kExtractHere:
  ...
  {
    ::MessageBoxW(0, L"Error", L"EcoZip", MB_ICONERROR);
  }
  return S_OK;
}
