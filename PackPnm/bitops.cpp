...
{	
	...
	if ( mode == 0 ) {
		...
		if ( buffer != NULL ) {
			while ( ( i = (int)fread( buffer, sizeof( char ), BUFFER_SIZE, stdin ) ) > 0 )
...
{
	return (int)fwrite( from, tpsize, dtsize, fptr );
}
...
{
	return (int)fread( to, tpsize, dtsize, fptr );
}
