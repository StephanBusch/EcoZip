...
#include "CPP/Common/AutoPtr.h"
#include "CPP/Common/StringConvert.h"

#include "CPP/Windows/ErrorMsg.h"
#include "CPP/Windows/ProcessUtils.h"
...
static CFSTR kTempDirPrefix = FTEXT("7zeO");
...
HRESULT CContentsView::OpenAsArc(IInStream *inStream,
...
HRESULT CContentsView::OpenAsArc_Msg(IInStream *inStream,
    const CTempFileInfo &tempFileInfo,
    const UString &virtualFilePath,
    const UString &arcFormat,
    bool &encrypted,
...
HRESULT CContentsView::OpenAsArc_Name(const UString &relPath, const UString &arcFormat, bool &encrypted, bool showErrorMessage)
{
  CTempFileInfo tfi;
  tfi.RelPath = relPath;
...
HRESULT CContentsView::OpenAsArc_Index(int index, const wchar_t *type, bool showErrorMessage)
{
  ...
  if (res != S_OK)
  {
    RefreshTitle(); // in case of error we must refresh changed title of 7zFM
    return res;
  }
  RefreshListCtrl();
  return S_OK;
...
HRESULT CContentsView::OpenParentArchiveFolder()
{
  ...
  if (newFileInfo.Find(folderLink.FilePath))
  {
    if (folderLink.WasChanged(newFileInfo))
    {
      ...
      if (::MessageBoxW((HWND)*this, message, L"EcoZip", MB_OKCANCEL | MB_ICONQUESTION) == IDOK)
      {
            ...
            folderLinkPrev.UsePassword, folderLinkPrev.Password) != S_OK)
        {
          ::MessageBoxW((HWND)*this, MyFormatNew(IDS_CANNOT_UPDATE_FILE,
              fs2us(folderLink.FilePath)), L"EcoZip", MB_OK | MB_ICONSTOP);
          return S_OK;
        }
      }
    }
...
static HRESULT StartEditApplication(const UString &path, bool useEditor, HWND window, CProcess &process)
{
  ...
  if (res != SZ_OK)
    ::MessageBoxW(window, LangString(IDS_CANNOT_START_EDITOR), L"EcoZip", MB_OK  | MB_ICONSTOP);
  return res;
}
...
//   if (res == SZ_OK)
//     return;
//   ::MessageBoxW(_window, LangString(IDS_CANNOT_START_EDITOR), L"EcoZip", MB_OK  | MB_ICONSTOP);
// }
...
void CContentsView::EditItem(int index, bool useEditor)
{
  if (!_parentFolders.IsEmpty())
  {
...
void CContentsView::OpenFolderExternal(int index)
{
...
bool CContentsView::IsVirus_Message(const UString &name)
{
  UString name2;
...
void CContentsView::OpenItem(int index, bool tryInternal, bool tryExternal, const wchar_t *type)
{
  CDisableTimerProcessing disableTimerProcessing(*this);
...
HRESULT CContentsView::OnOpenItemChanged(UInt32 index, const wchar_t *fullFilePath,
...
LRESULT CContentsView::OnOpenItemChanged(LPARAM lParam)
{
    ...
    if (mainProcessWasSet)
    {
      if (tpi->WasChanged(newFileInfo))
      {
        ...
        if (tpi->ReadOnly)
        {
          ...
          ::MessageBoxW(g_HWND, m, L"EcoZip", MB_OK | MB_ICONSTOP);
          return 0;
        }
        {
          const UString message = MyFormatNew(IDS_WANT_UPDATE_MODIFIED_FILE, tpi->RelPath);
          if (::MessageBoxW(g_HWND, message, L"EcoZip", MB_OKCANCEL | MB_ICONQUESTION) == IDOK)
          {
...
void CContentsView::OpenItemInArchive(int index, bool tryInternal, bool tryExternal, bool editMode, bool useEditor, const wchar_t *type)
{
  ...
  DWORD tid;
  HANDLE th;
  if ((th = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)MyThreadFunction, tpi, 0, &tid)) == NULL)
    throw 271824;
...
bool CContentsView::IsPreviewable(int index)
{
  const wchar_t *name = NULL;
  unsigned nameLen = 0;
  if (_folderGetItemName == NULL)
    return false;
  _folderGetItemName->GetItemName(index, &name, &nameLen);
  UString itemName = name;
  int nExt = itemName.ReverseFind_Dot();
  if (nExt >= 0) {
    UString ext = itemName.Mid(nExt + 1, itemName.Len() - nExt - 1);
    wchar_t *imgs[] = {
      L"bmp", L"dds", L"gif", L"ico", L"jpe", L"jpeg", L"jpg", L"png", L"tif", L"tiff",
      // HD Photo
      L"jxr", L"hdp", L"wdp",
      // Raw image format
      L"3fr",
    ...
    for (int i = 0; i < sizeof(imgs) / sizeof(imgs[0]); i++) {
      if (ext.IsEqualTo_NoCase(imgs[i])) {
        return true;
      }
    }
  }
...
CMyComPtr<IFolderFolder> CContentsView::ExtractItemToTemp(int index)
{
  const UString name = GetItemName(index);
  const UString relPath = GetItemRelPath(index);
  ...
  if (m_pTempDir != NULL)
    delete m_pTempDir;
  m_pTempDir = new CTempDir;
  if (!m_pTempDir->Create(kTempDirPrefix))
    return NULL;
  FString tempDir = m_pTempDir->GetPath();
  FString tempDirNorm = tempDir;
  NName::NormalizeDirPathPrefix(tempDirNorm);
  const FString tempFilePath = tempDirNorm + us2fs(Get_Correct_FsFile_Name(name));
  ...
  HRESULT result = CopyTo(options, indices, &messages,
	  usePassword, password, false);
  if (SUCCEEDED(result)) {
    CMyComPtr<IFolderFolder> folder = ::GetFolder(options.folder);
    folder->LoadItems();
    return folder;
  }
  return NULL;
