...
const UInt64 kComplexInCommands = (UInt64)1 <<
  #ifdef UNDER_CE
    31;
  #else
    34;
...
const UInt64 kComplexInSeconds = 4;
...
const UInt32 kFilterUnpackSize = (48 << 10);

const unsigned kOldLzmaDictBits = 30;

const UInt32 kAdditionalSize = (1 << 16);
const UInt32 kCompressedAdditionalSize = (1 << 10);
const UInt32 kMaxLzmaPropSize = 5;
...
struct CEncoderInfo
{
  ...
  {
    unsigned threadID;
    thread[0] = (HANDLE)_beginthreadex(NULL, 0, &EncodeThreadFunction, this, 0, &threadID);
    if (thread[0] != NULL)
      return S_OK;
    return E_FAIL;
  }
      ...
      )
  {
    ...
    unsigned threadID;
    thread[index] = (HANDLE)_beginthreadex(NULL, 0, &DecodeThreadFunction, &decoder, 0, &threadID);
    ...
    CBenchProps *benchProps)
{
  ...
  if (numEncoderThreads > 1)
    for (i = 0; i < numEncoderThreads; i++)
      WaitForSingleObject(encoders[i].thread[0], INFINITE);
  #endif
  ...
  if (numDecoderThreads > 1)
    for (i = 0; i < numEncoderThreads; i++)
      ...
      {
        CEncoderInfo &encoder = encoders[i];
        WaitForSingleObject(encoder.thread[j], INFINITE);
...
const UInt32 kNumFreqCommands = 128;
...
//   { 17,  357,  145,   20, "LIBBSC:2" },
//   { 17,  357,  145,   20, "CSC32:2" },
  {  0,    0,    0,    0, NULL },
};

const CBenchHash g_Hash[] =
{
    ...
    UInt32 &res)
{
  ...
  if (numThreads > 1)
  {
    ...
    for (i = 0; i < numThreads; i++)
    {
      CFreqInfo &info = threads.Items[i];
      unsigned threadID;
      info.hThread = (HANDLE)_beginthreadex(NULL, 0, &FreqThreadFunction, &info, 0, &threadID);
    ...
    bool showFreq, UInt64 cpuFreq)
{
  ...
  if (numThreads > 1)
  {
    ...
    for (i = 0; i < numThreads; i++)
    {
      CCrcInfo &info = threads.Items[i];
      unsigned threadID;
      info.hThread = (HANDLE)_beginthreadex(NULL, 0, &CrcThreadFunction, &info, 0, &threadID);
      if (info.hThread == NULL)
        return E_FAIL;
      threads.NumThreads++;
    }
