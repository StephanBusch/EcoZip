...
BEGIN_MESSAGE_MAP(CButtonST, CButton)
	...
	ON_NOTIFY_EX_RANGE(TTN_NEEDTEXT, 0, 0xFFFF, &CButtonST::OnNeedTipText)
	ON_WM_CREATE()
	ON_WM_DESTROY()
END_MESSAGE_MAP()
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (m_csBitmaps[byIndex].hMask != NULL) {
	...
	if (bIsDisabled && m_bShowDisabledBitmap)
	{
		...
		if (m_csBitmaps[byIndex].hBitmapOpaque != NULL) {
			HBITMAP hOldBmp = (HBITMAP)::SelectObject(hdcBmpMem, m_csBitmaps[byIndex].hBitmapOpaque);
			::BitBlt(hDC, 0, 0, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
			::SelectObject(hdcBmpMem, hOldBmp);
		}
		else {
			if (m_csBitmaps[byIndex].hMask != NULL)
	...
	else
	{
		if (m_csBitmaps[byIndex].hMask != NULL) {
			...
			::AlphaBlend(pDC->m_hDC, rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight,
				hdcBmpMem, 0, 0, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, bf);
		}
	} // else
...
//
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut, BOOL bMaskNeed)
{
	...
	return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut, bMaskNeed);
} // End of SetBitmaps

// This function assigns PNG bitmaps to the button.
// Any previous icon or bitmap will be removed.
//
// Parameters:
//		[IN]	nBitmapIn
//				ID number of the PNG bitmap resource to show when the mouse is over the button.
//				Pass NULL to remove any bitmap from the button.
//		[IN]	crTransColorIn
//				Color (inside nBitmapIn) to be used as transparent color.
//		[IN]	nBitmapOut
//				ID number of the PNG bitmap resource to show when the mouse is outside the button.
//				Can be NULL.
//		[IN]	crTransColorOut
//				Color (inside nBitmapOut) to be used as transparent color.
//
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetPNGBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut, BOOL bMaskNeed)
{
	HBITMAP		hBitmapIn		= NULL;
	HBITMAP		hBitmapOut		= NULL;
	HINSTANCE	hInstResource	= NULL;

	CImageUtils::LoadBitmapFromPNG(nBitmapIn, hBitmapIn);
	...
	switch (nBitmapOut)
	{
		...
		case (int)BTNST_AUTO_DARKER:
			...
			break;
		default:
			CImageUtils::LoadBitmapFromPNG(nBitmapOut, hBitmapOut);
			break;
	} // if

	return SetBitmaps(hBitmapIn, crTransColorIn, hBitmapOut, crTransColorOut, bMaskNeed);
} // End of SetPNGBitmaps
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn, COLORREF crTransColorIn, HBITMAP hBitmapOut, COLORREF crTransColorOut, BOOL bMaskNeed)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	CDC *pDC = GetDC();
	...
	if (hBitmapIn)
	{
		...
		if (!bMaskNeed)
			CImageUtils::PremultiplyBitmapAlpha(*pDC, m_csBitmaps[0].hBitmap, NULL);
		// Get bitmap size
		...
		if (hBitmapOut)
		{
			...
			if (!bMaskNeed)
				CImageUtils::PremultiplyBitmapAlpha(*pDC, m_csBitmaps[1].hBitmap, NULL);
			// Get bitmap size
			...
			if (bMaskNeed) {
				...
				m_csBitmaps[1].hBitmapOpaque = ::CreateCompatibleBitmap(*pDC, csBitmapSize.bmWidth, csBitmapSize.bmHeight);
				CImageUtils::SelectOpaquePixels(*pDC, m_csBitmaps[1].hBitmap, m_csBitmaps[1].hBitmapOpaque, 150);
			} // if
		} // if
	} // if
...
void CButtonST::InitToolTip()
{
	if (m_pToolTip == NULL) {
		...
		CRect rt;
		GetClientRect(rt);
		m_pToolTip->AddTool(this, LPSTR_TEXTCALLBACK, rt, 1);
	}
} // End of InitToolTip
...
void CButtonST::ResetBackground()
{
	m_bmpBk.DeleteObject();
	m_dcBk.DeleteDC();
}
...
//
#ifdef	BTNST_USE_BCMENU
DWORD CButtonST::SetMenu(UINT nMenu, HWND hParentWnd, UINT nToolbarID, CSize sizeToolbarIcon, COLORREF crToolbarBk, BOOL bRepaint)
{
	BOOL	bRetValue = FALSE;
...
BOOL CButtonST::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
...
INT_PTR CButtonST::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	ASSERT_VALID(this);

	// check child windows first by calling CMFCBaseToolBar
	INT_PTR nHit = (INT_PTR)CButton::OnToolHitTest(point, pTI);
	if (nHit != -1)
		return nHit;
	...
	if (pTI != NULL)
	{
		...
		//AfxLoadString(m_nID, szFullText);
		AfxExtractSubString(strTipText, m_strToolTip, 1, '\n');
...
// 				}
		...
		CString strDescr = strTipText;

		CFrameWnd* pParent = GetParentFrame();
		if (pParent->GetSafeHwnd() != NULL)
		{
			//pParent->GetMessageString(m_nID, strDescr);
		}
...
int CButtonST::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
