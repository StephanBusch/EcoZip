...
void BCMenu::SynchronizeMenu(void)
{
	...
	for (i = 0; i<GetMenuItemCount(); ++i){
		...
		if (state&MF_POPUP){
			submenu = (UINT)GetSubMenu(i)->m_hMenu;
			mdata = FindMenuList(submenu);
			GetMenuString(i, string, MF_BYPOSITION);
			if (!mdata)mdata = NewODMenu(i,
			...
			else ModifyMenu(i, mdata->nFlags, nID, (LPCTSTR)mdata);
		}
		else{
			nID = GetMenuItemID(i);
			mdata = FindMenuList(nID);
			GetMenuString(i, string, MF_BYPOSITION);
			if (!mdata)mdata = NewODMenu(i, state | MF_BYPOSITION | MF_OWNERDRAW,
				nID, string);
			else{
				...
				ModifyMenu(i, mdata->nFlags, nID, (LPCTSTR)mdata);
			}
		}
...
BOOL BCMenuToolBar::LoadBitmap(LPCTSTR lpszResourceName)
{
	...
	if (hbmImageWell != NULL && !AddReplaceBitmap(hbmImageWell))
		return FALSE;
