...
HRESULT CContentsView::InitColumns()
{
  SaveListViewInfo();
  ...
  */
  ...
  if (!orderError)
    this->SetColumnOrderArray(_visibleColumns.Size(), columns);
...
void CContentsView::DeleteColumn(unsigned index)
{
  _visibleColumns.Delete(index);
  CListCtrl::DeleteColumn(index);
}

void CContentsView::AddColumn(const CPropColumn &prop)
{
  ...
  CListCtrl::InsertColumn(index, &column);
}


HRESULT CContentsView::RefreshListCtrl()
{
...
void CContentsView::GetSelectedNames(UStringVector &selectedNames)
{
...
void CContentsView::SaveSelectedState(CSelectedState &s)
{
  s.FocusedName.Empty();
  s.SelectedNames.Clear();
  s.FocusedItem = this->GetFocusedItem();
  {
    if (s.FocusedItem >= 0)
    {
...
HRESULT CContentsView::RefreshListCtrl(const CSelectedState &s)
{
...
HRESULT CContentsView::RefreshListCtrlSaveFocused()
{
...
void CContentsView::SetFocusedSelectedItem(int index, bool select)
{
  UINT state = LVIS_FOCUSED;
  if (select)
    state |= LVIS_SELECTED;
  this->SetItemState(index, state, state);
...
HRESULT CContentsView::RefreshListCtrl(const UString &focusedName, int focusedPos, bool selectFocused,
    const UStringVector &selectedNames)
{
  ...
  */
  ...
  this->SetItemCount(numItems + (showDots ? 1 : 0));
  ...
  if (this->GetItemCount() > 0 && cursorIndex >= 0)
    SetFocusedSelectedItem(cursorIndex, selectFocused);
  // DWORD tickCount3 = GetTickCount();
  SetSortRawStatus();
  this->SortItems(CompareItems, (LPARAM)this);
  // DWORD tickCount4 = GetTickCount();
  if (cursorIndex < 0 && this->GetItemCount() > 0)
  {
    if (focusedPos >= this->GetItemCount())
      focusedPos = this->GetItemCount() - 1;
...
void CContentsView::GetSelectedItemsIndices(CRecordVector<UInt32> &indices) const
{
  indices.Clear();
  /*
  int itemIndex = -1;
...
void CContentsView::GetOperatedItemIndices(CRecordVector<UInt32> &indices) const
{
  GetSelectedItemsIndices(indices);
  if (!indices.IsEmpty())
    return;
...
void CContentsView::GetAllItemIndices(CRecordVector<UInt32> &indices) const
{
  indices.Clear();
  UInt32 numItems;
...
void CContentsView::GetOperatedIndicesSmart(CRecordVector<UInt32> &indices) const
{
...
/*
void CContentsView::GetOperatedListViewIndices(CRecordVector<UInt32> &indices) const
{
  indices.Clear();
...
void CContentsView::EditItem(bool useEditor)
{
  if (!useEditor)
  {
...
void CContentsView::OpenFocusedItemAsInternal(const wchar_t *type)
{
  int focusedItem = this->GetFocusedItem();
  if (focusedItem < 0)
    return;
...
void CContentsView::OpenSelectedItems(bool tryInternal)
{
  CRecordVector<UInt32> indices;
  GetOperatedItemIndices(indices);
  if (indices.Size() > 20)
  ...
  {
    int realIndex = GetRealItemIndex(focusedItem);
    if (realIndex == kParentIndex && (tryInternal || indices.Size() == 0) && this->IsItemSelected(focusedItem))
...
UString CContentsView::GetItemName(int itemIndex) const
{
...
UString CContentsView::GetItemName_for_Copy(int itemIndex) const
{
  if (itemIndex == kParentIndex)
    return L"..";
  {
...
void CContentsView::GetItemName(int itemIndex, UString &s) const
{
  if (itemIndex == kParentIndex)
  {
    s = L"..";
...
UString CContentsView::GetItemPrefix(int itemIndex) const
{
  if (itemIndex == kParentIndex)
    return UString();
  NCOM::CPropVariant prop;
...
UString CContentsView::GetItemRelPath(int itemIndex) const
{
  return GetItemPrefix(itemIndex) + GetItemName(itemIndex);
}

UString CContentsView::GetItemRelPath2(int itemIndex) const
{
...
UString CContentsView::GetItemFullPath(int itemIndex) const
{
  return GetFsPath() + GetItemRelPath2(itemIndex);
}

bool CContentsView::GetItem_BoolProp(UInt32 itemIndex, PROPID propID) const
{
  NCOM::CPropVariant prop;
...
bool CContentsView::IsItem_Deleted(int itemIndex) const
{
  if (itemIndex == kParentIndex)
    return false;
...
bool CContentsView::IsItem_Folder(int itemIndex) const
{
  if (itemIndex == kParentIndex)
    return true;
...
bool CContentsView::IsItem_AltStream(int itemIndex) const
{
  if (itemIndex == kParentIndex)
    return false;
...
UInt64 CContentsView::GetItemSize(int itemIndex) const
{
  if (itemIndex == kParentIndex)
    return 0;
  if (_folderGetItemName)
...
void CContentsView::SaveListViewInfo()
{
  if (!_needSaveInfo)
    return;
  ...
  for (i = 0; i < _visibleColumns.Size(); i++)
  {
    ...
    if (!this->GetColumn(i, &winColumnInfo))
      throw 1;
...
// void CContentsView::OnRightClick(NMHDR *pNMHDR, LRESULT *pResult)
// {
// 	LPNMITEMACTIVATE pNMItemActivate = reinterpret_cast<LPNMITEMACTIVATE>(pNMHDR);
// 	if (pNMItemActivate->hdr.hwndFrom == HWND(*this))
// 		return;
// 	POINT point;
// 	::GetCursorPos(&point);
...
void CContentsView::ShowColumnsContextMenu(int x, int y)
{
  NWindows::CMenu menu;
  ...
  int menuResult = menu.Track(TPM_LEFTALIGN | TPM_RETURNCMD | TPM_NONOTIFY, x, y, *GetTopLevelParent());
...
void CContentsView::OnTimer(UINT_PTR nIDEvent)
{
  if (!_processTimer)
    return;
  if (!AutoRefresh_Mode)
