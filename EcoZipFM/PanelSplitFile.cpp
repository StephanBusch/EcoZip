...
//  unsigned destPanelIndex = (NumPanels <= 1) ? srcPanelIndex : (1 - srcPanelIndex);
  ...
  UString progressWindowTitle = L"EcoZip"; // LangString(IDS_APP_TITLE, 0x03000000);
  UString title = LangString(IDS_SPLITTING);

  progressDialog.ShowCompressionInfo = false;

  progressDialog.MainWindow = *GetTopLevelParent();
  ...
  if (spliter.Create(title, *GetTopLevelParent()) != 0)
    return;
  }
  RefreshTitle();
...
//  unsigned destPanelIndex = (NumPanels <= 1) ? srcPanelIndex : (1 - srcPanelIndex);
  ...
  if (fileInfo.Find(combiner.OutputPath))
  {
    ...
    if (combiner.Create(title, *GetTopLevelParent()) != 0)
      return;
  }
  RefreshTitle();
