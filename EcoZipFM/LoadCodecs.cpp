...
---------------
  ...
  The order of check:
    ...
    2) WIN32: directory for REGISTRY item [HKEY_*\Software\EcoZip\Path**]
...
-------------
  ...
  EcoZip doesn't use that feature. EcoZip uses the scheme:
...
#include "CPP/Windows/Registry.h"
#endif
...
static LPCTSTR kRegistryPath = TEXT("Software") TEXT(STRING_PATH_SEPARATOR) TEXT("EcoZipFM");
...
FString GetBaseFolderPrefixFromRegistry()
{
...
HRESULT CCodecs::Load()
{
  ...
  for (UInt32 i = 0; i < g_NumArcs; i++)
  {
    ...
    RINOK(LoadDllsFromFolder(baseFolder));
...
#ifdef EXTERNAL_CODECS
HRESULT CCodecs::LoadExternalCodecs(FString &baseFolder, FStringVector &codecDlls)
{
  Formats.Clear();
  Codecs.Clear();
  Hashers.Clear();

  FOR_VECTOR(i, codecDlls) {
    RINOK(LoadDll(baseFolder + codecDlls[i], false));
  }
