...
void CSystemPage::ChangeState(unsigned group, const CUIntVector &indices)
{
  ...
  for (i = 0; i < indices.Size(); i++)
  {
    ...
    int state = kExtState_EcoZip;
    if (mi.State == kExtState_EcoZip)
  ...
  if (counters[kExtState_Other] != 0)
    state = kExtState_Other;
  else if (counters[kExtState_EcoZip] != 0)
...
{
  UString s = L"\"";
  s += fs2us(NWindows::NDLL::GetModuleDirPrefix());
  s.AddAscii("EcoZipFM.exe\" \"%1\"");
  return s;
}
...
BOOL CSystemPage::OnApply()
{
  ...
  if (res != 0)
    ::MessageBoxW(*this, NWindows::NError::MyFormatMessage(res), L"EcoZip", MB_ICONERROR);
