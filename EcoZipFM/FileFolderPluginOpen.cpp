...
#include "CPP/7zip/UI/FileManager/PluginLoader.h"
#include "CPP/7zip/UI/FileManager/RegistryPlugins.h"
    ...
    bool &encrypted, UString &password)
{
  ...
  */
  ...
  FOR_VECTOR (i, plugins)
  {
    ...
    t.OpenCallbackSpec->ProgressDialog.MainTitle = L"EcoZip"; // LangString(IDS_APP_TITLE);
    t.OpenCallbackSpec->ProgressDialog.MainAddTitle = progressTitle + L' ';
    t.OpenCallbackSpec->ProgressDialog.WaitMode = true;

    {
      unsigned threadID;
      HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, &CThreadArchiveOpen::MyThreadFunction, &t, 0, &threadID);
      if (hThread == NULL)
        return E_FAIL;
      t.OpenCallbackSpec->StartProgressDialog(progressTitle, hThread);
    }
