...
public:
  ...
  static const size_t o1pos = o0pos + o0size;
  static const size_t o2pos = o1pos + o1size;
  static const size_t s2pos = o2pos + o2size;
  static const size_t s3pos = s2pos + o1size;
  static const size_t s4pos = s3pos + o1size;
  ...
  size_t processBit(TStream&stream, size_t bit, size_t*base_contexts, size_t ctx, size_t mixer_ctx){
    ...
    int mixer_p = table.squnsafe(stp);
    p = mixer_p;
    if (kUseSSE){
      if (kUseLZP){
  ...
  size_t processByte(TStream&stream, uint32_t c = 0){
    size_t base_contexts[inputs] = { o0pos };
    ...
    if (modelEnabled(kModelSparse23)){
      *(ctx_ptr++) = hash_lookup(hashFunc(p2, hashFunc(p1, 0x37220B98)));
    }
    if (modelEnabled(kModelSparse34)){
      *(ctx_ptr++) = hash_lookup(hashFunc(p3, hashFunc(p2, 0x651A833E)));
    }
    if (modelEnabled(kModelWordMask)){
      ...
      mask ^= 1u << 3u;
      mask ^= 1u << 29u;
      *(ctx_ptr++) = hash_lookup((hash_t)((owhash&mask) * 0xac2bb7a9 + 19299412415));
    }
    ...
    if (modelEnabled(kModelWord12)){
      *(ctx_ptr++) = hash_lookup(word_model.get01Hash(), false);
    }
