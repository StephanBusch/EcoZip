...
public:
  ...
  public:
    ...
    void read(Stream *stream) {
      ...
      uint64_t prev = 0;
      ranges_.resize((size_t)num_ranges);
      for (auto&r : ranges_) {
  ...
  void nextRange() {
    while (cur_pos_ >= cur_end_) {
      if (range_idx_ < num_ranges_) {
        ...
        ++file_idx_;
        if (file_idx_ >= (int)segments_->size()) return;
        auto *segs = &segments_->operator[](file_idx_);
...
forceinline OutputStream &operator <<(OutputStream &stream, uint8_t c) {
  stream.put(c);
  return stream;
}
forceinline OutputStream &operator <<(OutputStream &stream, int8_t c) {
  return stream << static_cast<uint8_t>(c);
}
forceinline OutputStream &operator <<(OutputStream &stream, uint16_t c) {
  return stream << static_cast<uint8_t>(c >> 8) << static_cast<uint8_t>(c);
}
forceinline OutputStream &operator <<(OutputStream &stream, int16_t c) {
  return stream << static_cast<uint16_t>(c);
}
inline OutputStream &operator <<(OutputStream &stream, uint32_t c) {
  return stream << static_cast<uint16_t>(c >> 16) << static_cast<uint16_t>(c);
}
inline OutputStream &operator <<(OutputStream &stream, int32_t c) {
  return stream << static_cast<uint32_t>(c);
}
inline OutputStream &operator <<(OutputStream &stream, uint64_t c) {
  return stream << static_cast<uint16_t>(c >> 32) << static_cast<uint16_t>(c);
}
inline OutputStream &operator <<(OutputStream &stream, int64_t c) {
  return stream << static_cast<uint64_t>(c);
}
inline OutputStream &operator <<(OutputStream &stream, float c) {
  return stream << *reinterpret_cast<uint32_t*>(&c);
}
inline OutputStream &operator <<(OutputStream &stream, double c) {
